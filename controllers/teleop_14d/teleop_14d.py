...
class LeaderFollowerController(Supervisor):
    def __init__(self):
        ...
        # Webots setup
        # ────────────────────────────────────────────────────────────────
        print("\nSO-ARM100 LEADER-FOLLOWER - LOW-DIM DATA COLLECTION")
        ...
        try:
            ...
            print("[!] randomization_controller not found")
    ...
    def save_data(self):
        ...
        data_dir = "/home/gd/Desktop/6d_on_dp_for_soarm100_pick_place_in_webots/diffusion_policy/data/so100_lowdim/14D/batch_2"
        ...
        try:
            print(f"[Keys info] Saving keys: {list(data_dict.keys())}")
            print(f"[State shape] State shape: {data_dict['obs/state'].shape}")
            print(f"[Action shape] Action shape: {data_dict['action'].shape}")
        ...
        if os.path.exists(zarr_path):
            ...
            print(f"\n\n[Sucess] Low-dimensional Zarr dataset created: {zarr_path}")
            print(f"\tFile size: {size_mb:.2f} MB")
            print(f"\tEpisodes: {len(self.episode_ends)}")
            print(f"\tTotal frames: {len(self.obs_state)}")
            print(f"\tRecording frequency: {1/self.record_interval} Hz")
            print(f"\tState dimension: {self.STATE_DIM}")
            print(f"\tAction dimension: {self.ACTION_DIM}")
            print(f"\tDataset saved to: {os.path.abspath(zarr_path)}")
    ...
    def randomize_environment(self):
        # randomize box and goal positions using AutoDetectRandomizer
        if self.randomizer:
            try:
    ...
    def connect_to_leader(self):
        for port in ['/dev/ttyACM0', '/dev/ttyUSB0']:
            try:
                ...
                print(f"[Leader] Connected to leader on {port}")
                return
            except Exception:
                pass
        print("[Leader] Leader not connected")
    ...
    def stop_serial_thread(self):
        ...
        print("\tStopped serial reading thread")
    ...
    def run(self):
        ...
        # FIX 1: initialize with follower positions
        prev_follower_positions = [
        self.safe_get_value(self.sensors[n]) for n in self.JOINT_NAMES
        ]
        # FIX 1 end
        ...
        while self.step(self.timestep) != -1:
            ...
            if self.leader_connected and (now - self.last_leader_update >= self.leader_update_interval):
                ...
                # FIX 1: get currnt follower state: to compute actions from follower state, not leader intent
                current_follower_positions = [
                    self.safe_get_value(self.sensors[n]) for n in self.JOINT_NAMES
                ]
                action = self.calculate_action(prev_follower_positions, current_follower_positions)
                # FIX 1 end
                ...
                # FIX 1: update previous follower positions
                prev_follower_positions = current_follower_positions[:]
                # FIX 1 end
    ...
    def task_success(self):
        ...
        try:
            ...
            if success and self.RECORDING and self.success_time is None:
                ...
                print(f"          Press 'S' to save this episode")
    
            return success
        except Exception as e:
            print(f"[!] task_success check failed: {e}")
            return False
